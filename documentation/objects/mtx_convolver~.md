...
The input signal \\(x_i[s]\\) of the input index \\(i\\) and sample index \\(s\\) is divided by Pd in blocks \\(x_b[s]=x[bL+s]\\) of the length \\(L\\) with the local sample index \\(s=0\dots L\\).  Ttogether with the previous block, it is being \\(2L\\) fast Fourier transformed for real-valued signals, at every DSP cycle indexed by \\(b\\):
$$
 X_{i,b}[k]=\mathrm{rFFT_{2L}}\\{[x_{b-1}[0]\dots x_{b-1}[L-1]], [x_{b}[0]\dots x_{b}[L-1]] \\}.
$$
The impulse responses \\(h_{i,o,c}[s]\\) for the inlet \\(i=0\dots m-1\\), outlet \\(o=0\dots n-1\\) and current/old \\(c=0,1\\) crossfade index are partitioned into blocks of \\(L\\) by the partition index \\(p=0\dots P-1\\) with \\(P=\lceil\mathrm{len}/L\rceil\\) and Fourier transformedafter zero-padding to the size \\(2L\\), and the frequency bin index \\(k=0\dots L\\) is relevant for real-valued signals:
$$
...
$$
  y_{o}[s]=w[s]\thinspace y_{o,0}[s+L]+(1-w[s])\thinspace y_{o,1}[s+L],
$$
or with the option \\(\mathrm{pow}\\) for power-complementary output crossfade:
$$
  y_{o}[s]=\sqrt{w[s]}\thinspace y_{o,0}[s+L]+\sqrt{1-w[s]}\thinspace y_{o,1}[s+L],
$$
