...
static void mtxbin_scalar_matrix (t_mtx_binscalar *x, t_symbol *s,
...
static void mtxbin_scalar_list(t_mtx_binscalar *x, t_symbol *s, int argc,
                               t_atom *argv)
{
  int n=argc;
  t_atom *m;
...
static void mtxbin_matrix(t_mtx_binmtx *x, t_symbol *s, int argc,
                          t_atom *argv)
{
  ...
  if ((col<1)||(row<1)) {
    post( MTXBIN_SHORTNAME ": invalid dimensions");
    return;
  }
  if (col*row>argc-2) {
    post( MTXBIN_SHORTNAME
          ":sparse matrix not yet suppandted : use \"mtx_check\"");
    return;
  }
  ...
  if(x->m2.row==1) {
    ...
    for(r=0; r<row; r++) {
      ...
      for(c=0; c<col; c++) {
#ifdef MTXBIN_GENERIC__INTEGEROP
        t_float f = (t_float)(atom_getint(m1) MTXBIN_GENERIC__OPERATOR atom_getint(
                                m2));
#else
...
static void mtxbin_float(t_mtx_binmtx *x, t_float fval)
{
  ...
  if (!m2->atombuffer) {
    post( MTXBIN_SHORTNAME ": operate on what ?");
    return;
  }
...
void MTXBIN_APPEND(_setup) (void)
{
  ...
  class_addmethod(MTXBIN_APPEND(_class), (t_method)mtxbin_matrix,
                  gensym("matrix"), A_GIMME, 0);
  class_addmethod(MTXBIN_APPEND(_class), (t_method)mtx_bin_matrix2,
                  gensym(""), A_GIMME, 0);
  class_addfloat (MTXBIN_APPEND(_class), mtxbin_float);
  class_addbang  (MTXBIN_APPEND(_class), mtx_binmtx_bang);

  MTXBIN_APPEND(_scalarclass) = class_new(gensym(MTXBIN_LONGNAME), 0,
  ...
  class_sethelpsymbol(MTXBIN_APPEND(_scalarclass),
                      gensym(MTXBIN_REALHELPNAME));

  class_addcreator((t_newmethod)mtxbin_new, gensym(MTXBIN_SHORTNAME),
                   A_GIMME,0);
}
