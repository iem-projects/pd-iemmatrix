...
static void mtx_bspline_resize(t_mtx_spline *x, int cols, int dim)
{
  ...
  if(x->x_x) {
    freebytes(x->x_x, size);
  }
  x->x_x=0;

  for(i=0; i<x->x_dimension; i++) {
    if(x->x_y&&x->x_y[i]) {
      freebytes(x->x_y[i], size);
      x->x_y[i]=0;
    }
    if(x->x_u&&x->x_u[i]) {
      freebytes(x->x_u[i], size);
      x->x_u[i]=0;
    }
    if(x->x_p&&x->x_p[i]) {
      freebytes(x->x_p[i], size);
      x->x_p[i]=0;
    }
  }
  if(x->x_y) {
    freebytes(x->x_y, x->x_dimension*sizeof(t_matrixfloat*));
  }
  x->x_y=0;
  if(x->x_u) {
    freebytes(x->x_u, x->x_dimension*sizeof(t_matrixfloat*));
  }
  x->x_u=0;
  if(x->x_p) {
    freebytes(x->x_p, x->x_dimension*sizeof(t_matrixfloat*));
  }
  x->x_p=0;

  if(x->x_result) {
    freebytes(x->x_result, x->x_dimension*sizeof(t_atom));
  }
  x->x_p=0;
...
static void mtx_bspline_matrix2(t_mtx_spline *X, t_symbol *s, int argc,
                                t_atom *argv)
{
  int row=0;
  int col=0;
...
static void mtx_bspline_float(t_mtx_spline *X, t_float f)
{
  ...
  for(j=0; j<dim; j++) {
    ...
    t_float ret=(t_float)(t*y[j][i+1]+(1-t)*y[j][i]+uji*uji*
                          (p[j][i+1]*t3+p[j][i]*t3i)/6.0);
...
{
  mtx_bspline_class = class_new(gensym("mtx_bspline"),
                                (t_newmethod)mtx_bspline_new, (t_method)mtx_bspline_free,
                                sizeof(t_mtx_spline), 0, A_NULL);
  /*  class_addmethod(mtx_bspline_class, (t_method)mtx_bspline_matrix, gensym("list"), A_GIMME, 0); */
  class_addmethod(mtx_bspline_class, (t_method)mtx_bspline_matrix2,
                  gensym(""), A_GIMME, 0);
