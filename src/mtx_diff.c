...
static void *newMTXdiff (t_symbol *s, int argc, t_atom *argv)
{
  ...
  mtx_diff_obj->list_outlet = outlet_new (&mtx_diff_obj->x_obj,
                                          gensym("matrix"));
...
void mtx_diff_setup (void)
{
  ...
  class_addmethod (mtx_diff_class, (t_method) mTXdiffMatrix,
                   gensym("matrix"), A_GIMME,0);
  class_addmethod (mtx_diff_class, (t_method) mTXSetdiffMode, gensym("mode"),
                   A_DEFSYMBOL,0);
  class_addmethod (mtx_diff_class, (t_method) mTXSetdiffDirection,
