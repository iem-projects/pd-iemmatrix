...
static void writeIndexedValuesIntoMatrix (int n, int *idx, t_atom *x,
    t_atom *y)
{
  for (; n--; idx++,x++)
    if (*idx) {
...
static void writeFloatIndexedIntoMatrix (int n, int *idx, t_float f,
    t_atom *y)
{
  for (; n--; idx++)
    if (*idx) {
...
static void *newMTXFill (t_symbol *s, int argc, t_atom *argv)
{
  ...
  mtx_fill_obj->list_outlet = outlet_new (&mtx_fill_obj->x_obj,
                                          gensym("matrix"));
  inlet_new(&mtx_fill_obj->x_obj, &mtx_fill_obj->x_obj.ob_pd,
            gensym("matrix"),gensym("fill_mtx"));
  inlet_new(&mtx_fill_obj->x_obj, &mtx_fill_obj->x_obj.ob_pd,
            gensym("matrix"),gensym("index"));
...
void mtx_fill_setup (void)
{
  ...
  class_addmethod (mtx_fill_class, (t_method) mTXFillMatrix,
                   gensym("matrix"), A_GIMME,0);
  class_addmethod (mtx_fill_class, (t_method) mTXBigMatrix,
                   gensym("fill_mtx"), A_GIMME,0);
  class_addmethod (mtx_fill_class, (t_method) mTXFillIndexMatrix,
