...
static void mtx_gauss_mulsub(t_matrixfloat*a, t_matrixfloat*b, int count,
                             t_matrixfloat f)
{
...
static void mtx_gauss_matrix(t_matrixobj *x, t_symbol *s, int argc,
                             t_atom *argv)
{
  ...
  for(i=0; i<row; i++) {
    ...
    /*if(nz)*/
    {
      ...
      if(nz != i) {
        mtx_gauss_xch(original+i*col+i, original+nz*col+i, col-i);
      }
  ...
  float2matrix(x->m.atombuffer, original);
...
{
  t_matrixobj *x = (t_matrixobj *)pd_new(mtx_gauss_class);
...
void mtx_gauss_setup(void)
{
  ...
  class_addbang  (mtx_gauss_class, matrixobj_bang);
