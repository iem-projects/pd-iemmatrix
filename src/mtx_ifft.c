...
static void deleteMtxIFFT (MtxIFFT *x)
{
  ...
  if (x->plan)   iemmatrix_fft_destroy_plan(x->plan);
...
static void resizeFFT(MtxIFFT *x, int rows, int columns) {
  ...
  if(columns != x->columns) {
    if(x->plan)
      iemmatrix_fft_destroy_plan(x->plan);
    x->plan = iemmatrix_ifft_plan_1d(columns, x->c_in, x->c_out);
  }
...
void mtx_ifft_setup (void)
{
  ...
  iemmatrix_fft_init(mtx_ifft_class);
}
