...
static void deleteMTXIndex (MTXindex *mtx_index_obj)
{
  if (mtx_index_obj->index_in) {
    freebytes (mtx_index_obj->index_in,
               sizeof(int)*(mtx_index_obj->index_size+2));
  }
  if (mtx_index_obj->list_out) {
    freebytes (mtx_index_obj->list_out,
...
static void *newMTXIndex (t_symbol *s, int argc, t_atom *argv)
{
  ...
  mtx_index_obj->list_outlet = outlet_new (&mtx_index_obj->x_obj,
                               gensym("matrix"));
...
static void writeIndexedValuesIntoList (int n, int *indx, t_atom *x,
                                        t_atom *y)
{
  for (; n--; indx++,y++)
    if (*indx) {
                            ...
                            int argc, t_atom *argv)
{
  ...
  /* main part */
  list_out += 2;
  setAtomListConstFloat (mtx_index_obj->index_size, list_out,
                         mtx_index_obj->fill_value);
  writeIndexedValuesIntoList (mtx_index_obj->index_size, indx,list_in,
                              list_out);
...
void mtx_index_setup (void)
{
  ...
  class_addmethod (mtx_index_class, (t_method) mTXIndexMatrix,
                   gensym("matrix"), A_GIMME,0);
  class_addmethod (mtx_index_class, (t_method) mTXIndexRightMatrix,
                   gensym(""), A_GIMME,0);
}
