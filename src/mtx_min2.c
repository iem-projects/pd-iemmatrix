...
static void mtx_min2scalar_matrix(t_mtx_binscalar *x, t_symbol *s,
...
static void mtx_min2scalar_list(t_mtx_binscalar *x, t_symbol *s, int argc,
                                t_atom *argv)
{
  int n=argc;
  t_atom *m;
  ...
  while(n--) {
    m->a_type = A_FLOAT;
    (m++)->a_w.w_float = (atom_getfloat(argv)<offset)?atom_getfloat(
                           argv):offset;
    argv++;
  }
...
static void mtx_min2_matrix(t_mtx_binmtx *x, t_symbol *s, int argc,
                            t_atom *argv)
{
...
static void *mtx_min2_new(t_symbol *s, int argc, t_atom *argv)
{
  if (argc) {
    ...
    if (argc>1) {
      pd_error(x, "[mtx_min2]: extra arguments ignored");
    }
...
{
  mtx_min2_class = class_new(gensym("mtx_min2"), (t_newmethod)mtx_min2_new,
                             (t_method)mtx_binmtx_free,
                             sizeof(t_mtx_binmtx), 0, A_GIMME, 0);
  class_addmethod(mtx_min2_class, (t_method)mtx_min2_matrix,
                  gensym("matrix"), A_GIMME, 0);
  class_addmethod(mtx_min2_class, (t_method)mtx_bin_matrix2, gensym(""),
                  A_GIMME, 0);
  class_addfloat (mtx_min2_class, mtx_min2_float);
  class_addbang  (mtx_min2_class, mtx_binmtx_bang);

  mtx_min2scalar_class = class_new(gensym("mtx_min2"), 0,
                                   (t_method)mtx_binscalar_free,
                                   sizeof(t_mtx_binscalar), 0, 0);
  class_addmethod(mtx_min2scalar_class, (t_method)mtx_min2scalar_matrix,
                  gensym("matrix"), A_GIMME, 0);
