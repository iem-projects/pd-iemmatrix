                                  ...
                                  t_atom *argv)
{
  ...
  if ((col!=x->m2.col)||(row!=x->m2.row)) {
    pd_error(x, "matrix dimension do not match");    /* LATER SOLVE THIS */
    return;
  }
...
static void mtx_divelement_float(t_mtx_binmtx *x, t_float f)
{
  ...
  if (!m2->atombuffer) {
    pd_error(x, "right-hand matrix missing");
    return;
  }
...
static void mtx_divscalar_list(t_mtx_binscalar *x, t_symbol *s, int argc,
                               t_atom *argv)
{
  int n=argc;
  t_atom *m;
...
{
  mtx_divelement_class = class_new(gensym("mtx_./"),
                                   (t_newmethod)mtx_div_new, (t_method)mtx_binmtx_free,
                                   sizeof(t_mtx_binmtx), 0, A_GIMME, 0);
  class_addmethod(mtx_divelement_class, (t_method)mtx_divelement_matrix,
                  gensym("matrix"), A_GIMME, 0);
  class_addmethod(mtx_divelement_class, (t_method)mtx_bin_matrix2,
                  gensym(""), A_GIMME, 0);
  class_addfloat (mtx_divelement_class, mtx_divelement_float);
  class_addbang  (mtx_divelement_class, mtx_binmtx_bang);

  mtx_divscalar_class = class_new(gensym("mtx_./"), 0,
                                  (t_method)mtx_binscalar_free,
                                  sizeof(t_mtx_binscalar), 0, 0);
  class_addmethod(mtx_divscalar_class, (t_method)mtx_divscalar_matrix,
                  gensym("matrix"), A_GIMME, 0);
