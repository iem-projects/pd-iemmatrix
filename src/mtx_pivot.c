...
static void mtx_pivot_matrix(t_mtx_pivot *x, t_symbol *s, int argc,
                             t_atom *argv)
{
  ...
  outlet_anything(x->post, gensym("matrix"), 2+col*col,
                  x->m_post.atombuffer);
...
static void *mtx_pivot_new(t_floatarg f)
{
  ...
  x->m.row = x->m.col = x->m_pre.row = x->m_pre.col = x->m_post.row =
                                         x->m_post.col = 0;
...
{
  mtx_pivot_class = class_new(gensym("mtx_pivot"),
                              (t_newmethod)mtx_pivot_new, (t_method)mtx_pivot_free,
                              sizeof(t_mtx_pivot), 0,
                              A_DEFFLOAT, 0);
  class_addmethod(mtx_pivot_class, (t_method)mtx_pivot_matrix,
