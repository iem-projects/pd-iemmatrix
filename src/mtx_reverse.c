...
static void deleteMTXreverse (MTXreverse *mtx_reverse_obj)
{
  if (mtx_reverse_obj->list_out) {
    freebytes (mtx_reverse_obj->list_out,
               sizeof(t_atom)*(mtx_reverse_obj->size+2));
  }
}
static void mTXSetReverseMode (MTXreverse *mtx_reverse_obj,
                               t_symbol *c_mode)
{
  ...
  case 'e':
  case 'E':
  case '.': /* "element" just revert the whole matrix as if it was a list */
    mtx_reverse_obj->reverse_mode = -1;
    break;
  default:
...
static void *newMTXreverse (t_symbol *s, int argc, t_atom *argv)
{
  ...
  mtx_reverse_obj->list_outlet = outlet_new (&mtx_reverse_obj->x_obj,
                                 gensym("matrix"));
...
void mtx_reverse_setup (void)
{
  ...
  class_addmethod (mtx_reverse_class, (t_method) mTXreverseMatrix,
                   gensym("matrix"), A_GIMME,0);
  class_addmethod (mtx_reverse_class, (t_method) mTXSetReverseMode,
