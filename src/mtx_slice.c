...
static void deleteMTXSlice (MTXslice *mtx_slice_obj)
{
  if (mtx_slice_obj->list_out) {
    freebytes (mtx_slice_obj->list_out,
...
static void *newMTXSlice (t_symbol *s, int argc, t_atom *argv)
{
  ...
  mtx_slice_obj->list_outlet = outlet_new (&mtx_slice_obj->x_obj,
                               gensym("matrix"));
  inlet_new(&mtx_slice_obj->x_obj, &mtx_slice_obj->x_obj.ob_pd,
            gensym("list"),gensym(""));
...
static void writeSliceIntoList (int slicerows, const int slicecols,
...
void mtx_slice_setup (void)
{
  ...
  class_addmethod (mtx_slice_class, (t_method) mTXSliceMatrix,
                   gensym("matrix"), A_GIMME,0);
  class_addmethod (mtx_slice_class, (t_method) mTXSliceIndexList, gensym(""),
                   A_GIMME,0);
}
