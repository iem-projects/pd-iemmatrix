...
static void deleteMTXSort (MTXSort *mtx_sort_obj)
{
  if (mtx_sort_obj->list_out1) {
    freebytes (mtx_sort_obj->list_out1,
               sizeof(t_atom)*(mtx_sort_obj->size+2));
  }
  if (mtx_sort_obj->list_out2) {
    freebytes (mtx_sort_obj->list_out2,
               sizeof(t_atom)*(mtx_sort_obj->size+2));
  }
  if (mtx_sort_obj->x) {
...
static void *newMTXSort (t_symbol *s, int argc, t_atom *argv)
{
  ...
  mtx_sort_obj->list_outlet1 = outlet_new (&mtx_sort_obj->x_obj,
                               gensym("matrix"));
  mtx_sort_obj->list_outlet2 = outlet_new (&mtx_sort_obj->x_obj,
                               gensym("matrix"));
...
void mtx_sort_setup (void)
{
  ...
  class_addmethod (mtx_sort_class, (t_method) mTXSortMatrix,
                   gensym("matrix"), A_GIMME,0);
  class_addmethod (mtx_sort_class, (t_method) mTXSetSortMode, gensym("mode"),
                   A_DEFSYMBOL,0);
  class_addmethod (mtx_sort_class, (t_method) mTXSetSortDirection,
